...
@section('main')
    ...
    <script>
        document.getElementById('showProfilesButton').addEventListener('click', function() {
            ...
            <div class="row box-j">
                <div class="col-lg-4 p0">
                   <a href="/customer/details/${item.id}">
                       <img src="/storage/${item.details?.image_path || 'default/image.jpg'}" class="img-fluid wid-testimoni">
                   </a>
                </div>
                <div class="col-lg-8 p0">
                    <div class="test-main cls-for-all">
                        <strong>${item.customer_id}</strong>
                        <p>Last seen on ${formatDate(item.last_login_time)}</p>
                        <h4 class="pro-hea">${item.name}</h4>
                        <p>${item.details?.height || 'N/A'} . ${item.details?.locations || 'N/A'}/${item.details?.permanent_locations || 'N/A'} . Others</p>
                        <ul class="list-none">
                            <li><span>Business - Beautician</span></li>
                            <li><span>Rs. ${item.details?.annual_income || 'N/A'} p.a</span></li>
                            <li><span>${item.details?.qualification || 'N/A'}</span></li>
                            <li><span>${item.details?.marritialstatus || 'N/A'}</span></li>
                        </ul>
                    </div>
                    <div class="pink-bg-list">
                        <ul class="list-none col-li">
